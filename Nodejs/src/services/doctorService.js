               ...
               let data = await db.User.findOne({
                   ...
                   include: [
                    ...
                    {model: db.Doctor_Infor,
                        ...
                        include: [
                            {model: db.Allcode,
                                as:'priceTypeData',attributes: ['valueEn','valueVi'] },
                            {model: db.Allcode,
                                as:'paymentTypeData',attributes: ['valueEn','valueVi'] },
                            {model: db.Allcode,
                                as:'provinceTypeData',attributes: ['valueEn','valueVi'] },
                        ]
                    }
                   ],
                   raw:false,
...
let getListPatientForDoctor = (inputId,date) => {
    return new Promise(async(resolve,reject) => {
        try {
            if(!inputId || !date) {
                resolve({
...
let sendRemedy = (data) => {
    return new Promise(async(resolve,reject) => {
        try {
            if(!data.email || !data.patientId || !data.doctorId || !data.timeType || !data.imageBase64) {
                resolve({
                    errCode:1,
                    errMessage: 'Missing parameter'
                })
